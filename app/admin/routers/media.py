...
async def upload_media_file(
    ...
    if not image.filename:
        return RedirectResponse(
            url="/admin/media?error=Файл+не+выбран",
            status_code=302,
        )
    ...
    if not content_type.startswith("image/"):
        return RedirectResponse(
            url="/admin/media?error=Можно+загружать+только+изображения",
            status_code=302,
        )
    ...
    if not data:
        return RedirectResponse(
            url="/admin/media?error=Файл+пустой",
            status_code=302,
        )
    if len(data) > 10 * 1024 * 1024:
        return RedirectResponse(
            url="/admin/media?error=Файл+слишком+большой+(максимум+10MB)",
            status_code=302,
        )
    ...
    if not bot:
        return RedirectResponse(
            url="/admin/media?error=BOT_TOKEN+не+настроен",
            status_code=302,
        )
    if not settings.admin_tg_id:
        return RedirectResponse(
            url="/admin/media?error=ADMIN_TG_ID+не+настроен",
            status_code=302,
        )
    ...
    if not file_id:
        return RedirectResponse(
            url="/admin/media?error=Не+удалось+получить+file_id",
            status_code=302,
        )
    ...
    return RedirectResponse(url="/admin/media?msg=Изображение+загружено", status_code=302)
